...
"""
Django settings for hospitool project.
...
DEBUG = bool(strtobool(os.environ.get("HOSPITOOL_DEBUG", "False")))

# SECURITY WARNING: keep the secret key used in production secret!
SECRET_KEY = os.environ.get("HOSPITOOL_SECRET_KEY", "dummy" if DEBUG else "")

BASE_URL = os.environ.get("HOSPITOOL_BASE_URL", "http://localhost:8086")
...
ALLOWED_HOSTS = [".localhost", "127.0.0.1", "[::1]", HOSTNAME] + [
    x.strip() for x in os.environ.get("HOSPITOOL_ALLOWED_HOSTS", "").splitlines() if x
]
...
MIDDLEWARE = [
    ...
    "hospitool.core.middleware.AccessControlMiddleware",
    "hospitool.core.middleware.TimetravelMiddleware",
]

ROOT_URLCONF = "hospitool.core.urls"
...
TEMPLATES = [
    {
        ...
        "OPTIONS": {
            "context_processors": [
                ...
                "hospitool.core.theme.context_processors.theme",
                "hospitool.core.context_preprocessors.global_context",
            ]
        },
    }
]

WSGI_APPLICATION = "hospitool.core.wsgi.application"
...
DATABASES = {
    "default": {
        ...
        "NAME": os.environ.get("HOSPITOOL_DB_NAME", "hospitool"),
        "USER": os.environ.get("HOSPITOOL_DB_USER", "hospitool"),
        "PASSWORD": os.environ.get("HOSPITOOL_DB_PASSWORD", "password" if DEBUG else ""),
        "HOST": os.environ.get("HOSPITOOL_DB_HOST", "localhost"),
        "PORT": os.environ.get("HOSPITOOL_DB_PORT", "5432"),
    }
}
...
STATIC_ROOT = os.environ.get("HOSPITOOL_STATIC_ROOT")
...
MEDIA_ROOT = os.environ.get("HOSPITOOL_MEDIA_ROOT", os.path.join(BASE_DIR, "media"))

#: The maximum size of media files in bytes
MEDIA_MAX_UPLOAD_SIZE = int(
    os.environ.get("HOSPITOOL_MEDIA_MAX_UPLOAD_SIZE", 3 * 1024 * 1024)
)
...
#: The log level for hospitool django apps
LOG_LEVEL = os.environ.get("HOSPITOOL_LOG_LEVEL", "DEBUG" if DEBUG else "INFO")

#: The file path of the logfile. Needs to be writable by the application.
LOGFILE = os.environ.get("HOSPITOOL_LOGFILE", os.path.join(BASE_DIR, "hospitool.log"))
...
LOGGING = {
    ...
    "loggers": {
        "hospitool": {"handlers": ["console-colored", "logfile"], "level": LOG_LEVEL},
        "hospitool.core.management.commands": {
...
DEFAULT_FROM_EMAIL = os.environ.get("HOSPITOOL_SERVER_EMAIL", "noreply@example.com")

#: The email address that error messages come from, such as those sent to :attr:`~hospitool.core.settings.ADMINS`.
#: (see :setting:`django:SERVER_EMAIL`)
SERVER_EMAIL = os.environ.get("HOSPITOOL_SERVER_EMAIL", "noreply@example.com")

#: A list of all the people who get code error notifications. When :attr:`~hospitool.core.settings.DEBUG` is ``False``,
#: Django emails these people the details of exceptions raised in the request/response cycle.
ADMINS = [("HospiTool Helpdesk", "tech@hospitool.de")]

#: The host to use for sending email (see :setting:`django:EMAIL_HOST`)
EMAIL_HOST = os.environ.get("HOSPITOOL_EMAIL_HOST", "localhost")

#: Password to use for the SMTP server defined in :attr:`~hospitool.core.settings.EMAIL_HOST`
#: (see :setting:`django:EMAIL_HOST_PASSWORD`). If empty, Django won’t attempt authentication.
EMAIL_HOST_PASSWORD = os.environ.get("HOSPITOOL_EMAIL_HOST_PASSWORD")

#: Username to use for the SMTP server defined in :attr:`~hospitool.core.settings.EMAIL_HOST`
#: (see :setting:`django:EMAIL_HOST_USER`). If empty, Django won’t attempt authentication.
EMAIL_HOST_USER = os.environ.get("HOSPITOOL_EMAIL_HOST_USER", SERVER_EMAIL)

#: Port to use for the SMTP server defined in :attr:`~hospitool.core.settings.EMAIL_HOST`
#: (see :setting:`django:EMAIL_PORT`)
EMAIL_PORT = int(os.environ.get("HOSPITOOL_EMAIL_PORT", 587))
...
EMAIL_USE_TLS = bool(strtobool(os.environ.get("HOSPITOOL_EMAIL_USE_TLS", "True")))
...
EMAIL_USE_SSL = bool(strtobool(os.environ.get("HOSPITOOL_EMAIL_USE_SSL", "False")))
