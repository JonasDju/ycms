...
from .constants.gender import DIVERSE, FEMALE, MALE
from .models import (
    Bed,
    BedAssignment,
    Floor,
...
def import_data(csv_file, user, data_to_import, val_sep=","):
    ...
    category_columns = {
        "patient": [
            'last_name", "first_name", "insurance_type", "gender", "age_at_arrival'
        ],
    ...
    if missing_columns:
        logger.error(
            "CSV file does not contain all required column labels for selected categories!"
        )
    ...
    updated_entries = {"ward": [], "bed": []}

    duplicate_count = (
        0  # number of entries in the file that already existed in the database
    )
    ...
    for index, patient_entry in df.iterrows():
        ...
        newly_created_medical_record = (
            newly_created_bed_assignment
        ) = updated_bed_assignment = False
        try:
            ...
            year_of_birth = (
                datetime.now().year - patient_entry[column_name["age_at_arrival"]]
            )
            ...
            created_at = datetime.strptime(
                patient_entry[column_name["created_at"]], "%d.%m.%Y %H:%M"
            )  # if creation_date else import_time
            ...
            if to_import["medical_record"]:
                (
                    imported_diagnosis_code,
                    newly_created_diagnosis_code,
                ) = ICD10Entry.objects.get_or_create(
                    code=patient_entry[column_name["diagnosis_code"]],
                    defaults={"description": ""},
                )
                if newly_created_diagnosis_code:
                    imported_entries["diagnosis_code"].append(
                        imported_diagnosis_code.pk
                    )
                    logger.debug(
                        f"Created new diagnosis code (pk: {imported_diagnosis_code.pk})"
                    )
                else:
                    logger.debug(
                ...
                if newly_created_medical_record:
                    imported_entries["medical_record"].append(
                        imported_medical_record.pk
                    )
                    logger.debug(
                        f"Created new medical record (pk: {imported_medical_record.pk})"
                    )
                else:
                    logger.debug(
                ...
                if to_import["bed_assignment"]:
                    ...
                    admission_date = datetime.strptime(
                        patient_entry[column_name["admission_date"]], "%d.%m.%Y %H:%M"
                    )
                    discharge_date = datetime.strptime(
                        patient_entry[column_name["discharge_date"]], "%d.%m.%Y %H:%M"
                    )

                    (
                        imported_bed_assignment,
                        newly_created_bed_assignment,
                    ...
                    if (
                        to_import["ward"]
                        and imported_bed_assignment.recommended_ward == None
                    ):
                        ...
                        ward = Ward.objects.get(floor=floor, name=ward_name)
                    elif to_import["bed"]:
                    ...
                    if to_import["bed"] and imported_bed_assignment.bed == None:
                        ...
                        room = Room.objects.get(room_number=room_number, ward=ward)
                        ...
                        for bed in beds:
                            ...
                            raise Exception(
                                f"Found no available bed with bed_type {bed_type} in {room}"
                            )
                    ...
                    if newly_created_bed_assignment:
                        ...
                        imported_entries["bed_assignment"].append(
                            imported_bed_assignment.pk
                        )
                        logger.debug(
                            f"Created new bed assignment (pk: {imported_bed_assignment.pk})"
                        )
                    else:
            ...
            ICD10Entry.objects.filter(
                pk__in=imported_entries["diagnosis_code"]
            ).delete()
            MedicalRecord.objects.filter(
                pk__in=imported_entries["medical_record"]
            ).delete()
            BedAssignment.objects.filter(
                pk__in=imported_entries["bed_assignment"]
            ).delete()
            # If bed_assignments were updated (existing bed_assignment with value None for ward and/or bed), reset updated fields to None
            BedAssignment.objects.filter(pk__in=updated_entries["ward"]).update(
            ...
            result["error_msg"] = _(
                "An error occured when importing entry no. {}."
            ).format(result["error_code"])
            result["duplicate_entry_count"] = result["updated_entry_count"] = 0
            return result
        if not newly_created_patient:
            if (
                not newly_created_medical_record
                and not newly_created_bed_assignment
                and not updated_bed_assignment
            ):
                duplicate_count += 1
            else:
                update_count += 1
